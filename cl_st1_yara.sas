...
DATA activity ;
  INFILE "/home/&sasusername/&myfolder/users.txt" ;
  length posts 8 user $ 8 ;
  input posts user $ ;
RUN;

proc rank data = activity groups = 10 out = activity_ranked ties=dense ; 
var posts ; 
ranks post_rank ;
run;

data activity_ranked  ;
    set activity_ranked ;
    array v post_rank  ;
    do over v ; 
      v = v + 1  ; 
...
ods html file="&whereisit/&myfolder/user_activity_ranks.html"; 
proc means data=activity_ranked min max mean n ; 
var posts ;
class post_rank; 
run;
ods html close;
...
DATA iduser ;
  INFILE "/home/&sasusername/&myfolder/id_user.txt" ;
  length file $ 19 user $ 8 ;
  input file $ user $ ;
RUN;

data activity_ranked;
merge activity_ranked (in=a) iduser (in=b) ;
by user;
...
var followers ; 
ranks followers_rank ;
run;

data popularity_ranked  ;
    set popularity_ranked ;
    array v followers_rank  ;
    do over v ; 
      v = v + 1  ; 
    end ;
run;

ods html file="&whereisit/&myfolder/user_popularity_ranks.html"; 
proc means data=popularity_ranked min max mean n ; 
var followers ;
...
data popularity_ranked;
merge popularity_ranked (in=a) iduser (in=b) ;
by user;
if (a and b) then output;
proc sort; by file;
run;

/* message virality : replies, likes */

DATA virality ;
  INFILE "/home/&sasusername/&myfolder/replies_likes.txt" ;
  length file $ 19 replies 8 likes 8;
  input file $ replies likes ;
RUN;

proc rank data = virality groups = 10 out = virality_ranked ties=dense ; 
var replies likes ; 
ranks replies_rank likes_rank ;
run;

data virality_ranked  ;
    set virality_ranked ;
...
ods html file="&whereisit/&myfolder/message_virality_ranks.html"; 
proc means data=virality_ranked min max mean n ; 
var replies ;
class replies_rank; 
run;
proc means data=virality_ranked min max mean n ; 
...
DATA wcount ;
  ...
  length file $ 19 wcount 8;
  input file $ wcount ;
  proc sort ; by file;
RUN;

proc rank data = wcount groups = 4 out = wcount_ranked ties=dense ; 
var wcount ; 
ranks wcount_rank ;
run;
...
ods html file="&whereisit/&myfolder/message_length_ranks.html"; 
proc means data=wcount_ranked min max mean n ; 
var wcount ;
class wcount_rank; 
run;
ods html close;
...
DATA topics ;
  INFILE "/home/&sasusername/&myfolder/topics.txt" ;
  length file $ 19 topicscore 8 topic $ 8 ;
  input file $ topicscore topic $ ;
  proc sort; by file ;
RUN;
...
DATA conversations ;
  INFILE "/home/&sasusername/&myfolder/conversations.txt" ;
  length file $ 19 conversation $ 8 ;
  input file $ conversation $ ;
  proc sort; by file ;
RUN;
...
DATA topconversations ;
  INFILE "/home/&sasusername/&myfolder/conversations_top_10.txt" ;
  length topconv $ 8  file $ 19 ;
  input topconv $  file $  ;
  proc sort; by file ;
RUN;
...
data temp;
merge activity_ranked (in=a) scores (in=b) ;
by file;
proc sort; by file;
run;

data temp;
merge temp (in=a) popularity_ranked (in=b) ;
by file;
proc sort; by file;
run;

data temp;
merge temp (in=a) virality_ranked (in=b) ;
by file;
proc sort; by file;
run;

data temp;
merge temp (in=a) dates (in=b) ;
by file;
proc sort; by file;
run;

data temp;
merge temp (in=a) wcount_ranked (in=b) ;
by file;
proc sort; by file;
run;

data temp;
merge temp (in=a) topics (in=b) ;
by file;
proc sort; by file;
run;

data temp;
merge temp (in=a) conversations (in=b) ;
by file;
proc sort; by file;
run;

data temp;
merge temp (in=a) topconversations (in=b) ;
by file;
proc sort; by file;
run;
...
data scores_metadata ;
 retain file &factorvars user posts post_rank followers followers_rank replies replies_rank likes likes_rank 
 wcount wcount_rank month year topic topicscore conversation topconv v000001-v001000 ;
 set scores_metadata ;
run; 
...
%repeat_glm(var=post_rank)
%repeat_glm(var=likes_rank)
%repeat_glm(var=replies_rank)
%repeat_glm(var=wcount_rank)
%repeat_glm(var=topic)
...
ods html file="&whereisit/&myfolder/glm_meta_top_conversations.html"; 
%macro create(howmany);
%do i=1 %to &howmany;
ods graphics off; 
%let var = topconv ;
...
set WORK.rsq_followers_rank_&first.-WORK.rsq_followers_rank_&last ;
...
data temp (KEEP= Factor RSquare Percent);
retain Factor RSquare Percent;
set WORK.rsq_post_rank_&first.-WORK.rsq_post_rank_&last ;
Factor = substr(Dependent, 4, 1);
Percent = RSquare * 100;
run;
...
data temp (KEEP= Factor RSquare Percent);
retain Factor RSquare Percent;
set WORK.rsq_likes_rank_&first.-WORK.rsq_likes_rank_&last ;
Factor = substr(Dependent, 4, 1);
Percent = RSquare * 100;
run;
...
data temp (KEEP= Factor RSquare Percent);
retain Factor RSquare Percent;
set WORK.rsq_replies_rank_&first.-WORK.rsq_replies_rank_&last ;
Factor = substr(Dependent, 4, 1);
Percent = RSquare * 100;
run;
...
data temp (KEEP= Factor RSquare Percent);
retain Factor RSquare Percent;
set WORK.rsq_wcount_rank_&first.-WORK.rsq_wcount_rank_&last ;
Factor = substr(Dependent, 4, 1);
Percent = RSquare * 100;
run;

title "Messsage length" ;
proc print data= temp NOOBS; format RSquare 8.3 ; run;
title ;

data temp (KEEP= Factor RSquare Percent);
retain Factor RSquare Percent;
set WORK.rsq_topic_&first.-WORK.rsq_topic_&last ;
Factor = substr(Dependent, 4, 1);
Percent = RSquare * 100;
run;
...
data temp (KEEP= Factor RSquare Percent);
retain Factor RSquare Percent;
set WORK.rsq_month_&first.-WORK.rsq_month_&last ;
Factor = substr(Dependent, 4, 1);
Percent = RSquare * 100;
run;
...
data temp (KEEP= Factor RSquare Percent);
retain Factor RSquare Percent;
set WORK.rsq_topconv_&first.-WORK.rsq_topconv_&last ;
Factor = substr(Dependent, 4, 1);
Percent = RSquare * 100;
run;

title "Conversations (top 10 with most messages)" ;
proc print data= temp NOOBS; format RSquare 8.3 ; run;
title ;
...
proc sgplot data=scores_metadata;
  vbar &var / response=fac&i stat=mean 
            ...
            datalabel = fac&i datalabelfitpolicy=rotate ;
  title height=12pt "Mean dim. &i scores (%qscan(&var,1,_))";
  yaxis label= "Mean dimension &i score" ;
  xaxis label= "Ranks of %qscan(&var,1,_)";
...
run;
%mend repeat_do;
%repeat_do(var=post_rank)
%repeat_do(var=wcount_rank)
%end;
%mend create;
%create( &extractfactors ) /* number of factors extracted */ 
quit;
...
%macro repeat_do(var=);
ods listing gpath="&whereisit/&myfolder/";
ods graphics on / reset imagename="&var._dim_&i._limits" imagefmt=png;
proc sql noprint;
    select rsquare into :names separated by ' ' from rsq_&var._fac&i ;
quit;
data temp;
...
/* mean dimension scores charts for topics */

data temp (DROP = temp temp2);
set scores_metadata ;
temp = substr(topic, 2, 2);
  topicn = input(temp, 8.);
  rename topic = temp2;
  rename topicn = topic ;
run;
...
run;
proc sql noprint;
    select percent into :perc separated by ' ' from temp2 ;
quit;
ods listing gpath="&whereisit/&myfolder/";
ods graphics on / reset imagename="&var._dim_&i" imagefmt=png ;
...
proc sgplot data=scores_metadata;
  vbar &var / response=fac&i stat=mean 
            ...
            baselineattrs=(thickness=1 color=red) 
            datalabel = fac&i ;
  title height=12pt "Mean scores dim. &i (&var)";
  yaxis label= "Mean dimension &i score" ;
  INSET  ( "R(*ESC*){sup '2'}" = "&names" "%" = "&perc" ) / BORDER TEXTATTRS = (SIZE=10 COLOR=black) OPAQUR ;
run;
%mend repeat_do;
%repeat_do(var=topconv)
%end;
%mend create;
%create( &extractfactors ) /* number of factors extracted */ 
quit;
...
data covar&i (KEEP= followperc -- unaccountedperc ); 
  set covar&i; 
  sumcovar=(followers_rank + post_rank + likes_rank + 
  replies_rank + wcount_rank + topic + month + year + residual);
  followperc=( ( followers_rank / sumcovar ) * 100 );
  postperc=( ( post_rank / sumcovar ) * 100 );
  likesperc=( (likes_rank / sumcovar ) * 100 );
  repliesperc=( (replies_rank / sumcovar ) * 100 );
  wcountperc=( (wcount_rank / sumcovar ) * 100 );
  topicperc=( (topic / sumcovar ) * 100 );
  monthperc=( (month / sumcovar ) * 100 );
  yearperc=( (year / sumcovar ) * 100 );
  unaccountedperc=( (residual/ sumcovar ) * 100 );
  
    label followperc = 'Followers' 
        postperc = 'Posts' 
        likesperc = 'Likes'         
        repliesperc = 'Replies' 
        wcountperc = 'Messg. Length' 
        topicperc = 'Topic'         
        monthperc = 'Month'  
        yearperc = 'Year'  
...
proc sgplot data=size_&var ;
  ...
  yaxis label= "Texts"  ranges=(min-12000 60000-max);
...
proc sgplot data=size_&var ;
  ...
  yaxis label= "Words" ranges=(min-250000 1250000-max) ;
  xaxis label= "Year" labelattrs=(size=10);
run;
%mend repeat_do;
%repeat_do(var=year)
...
proc sgplot data=size_&var ;
  ...
  title height=12pt "Corpus size by &var (words)";
  yaxis label= "Words" ranges=(min-200000 400000-max) ;
  xaxis label= "Month" labelattrs=(size=10);
run;
%mend repeat_do;
%repeat_do(var=month)
...
OPTIONS VALIDVARNAME=ANY;
FILENAME IN "/home/&sasusername/group4_profiles/group4_profiles_scores_only.tsv";
